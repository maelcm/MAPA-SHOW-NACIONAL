    ...
    if m_id:
        ...
        if not row.empty:
            ...
            if status == "Livre":
                ...
                b1, b2 = st.columns(2)
                if b1.button("💾 Salvar reserva", type="primary", width="stretch"):
                    if not (cli or "").strip():
                ...
                if b1.button("💲 Marcar pago", type="primary", width="stretch"):
                    atualizar_status(d["ID_Venda"], "Vendido", d["Preco_Num"])
                if b2.button("❌ Cancelar reserva", width="stretch"):
                    cancelar_reserva(d["ID_Venda"])
                if b3.button("Fechar", width="stretch"):
                    st.session_state["mesa_id"] = None
                    st.rerun()
            elif status == "Vendido":
                st.success(f"Vendido para **{d['Nome_Cliente']}**")
                b1, b2 = st.columns(2)
                if b1.button("Desfazer venda", width="stretch"):
